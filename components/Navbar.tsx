...
export default function Navbar({ user, onLogout }: NavbarProps) {
  ...
  const switchRole = async (role: string) => {
    try {
      ...
      const isDemo = user?.email === 'demo@fitrahpro.com';
      const endpoint = isDemo ? '/api/auth/demo-switch-role' : '/api/auth/dev-switch-role';
    ...
    <nav className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between relative">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          {user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN' || user?.email === 'demo@fitrahpro.com' ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-52">
                <DropdownMenuLabel className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">
                  {user?.email === 'demo@fitrahpro.com' ? 'Simulasi Role Demo' : 'Simulasi Role (Admin Only)'}
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                {[
